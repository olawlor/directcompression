...
public:
    ...
    */
    ...
    int          get_top_level_glut_window_id( void ) { return top_level_glut_window_id; } /* JVK */
};
...
/** Onscreen coordinates */
    ...
    int             x_abs, y_abs;                /* GLUT subwindow absolute coordinates */
...
public:
    ...
    */
    ...
    /** Draw this control to the screen.  The OpenGL coordinate system
      has already been translated to pixel (x,y) onscreen.
      This routine must be overridden by each GLUI_Control subclass. */
    virtual void draw( int x, int y )=0;
    
    /** Draw us, then draw all our children (according to GLUI_Node) */
    void         draw_recursive( int x, int y );
    
    /** Shift the OpenGL coordinate system, and call draw */
    void         translate_and_draw_front( void );
    
    /** Shift the OpenGL coordinate system from panel coords to our control's coords. */
...
/* Utility routines to help you draw controls: */
    /** Draw our name string, greyed out if we're not enabled. */
...
/**
 An EditText is a single-line text box.  It's used by Spinner, 
 CommandLine, and several other places.
 
 For multi-line text, see GLUI_TextBox below.
*/
...
/**
  A RadioGroup contains a set of RadioButton objects,
  only one of which can be selected at once.
  
  This class provides the interface to get and set the currently
  selected button index.  You use it by creating a RadioGroup, and 
  then adding a set of RadioButtons to it.
*/
...
/**
  A RadioButton is like a checkbox, but only one button in the 
  RadioGroup can be selected at once.
*/
...
/**
  A Separator is just used to provide space between components.
  It doesn't do anything other than take up space onscreen.
*/
...
/**
  A spinner provides an EditText box and a set of arrows.
  You can flip through a set of integer or float values with the arrows,
  or enter a new value in the tex box.
*/
...
/**
  StaticText is a one-line visible non-editable text class.  
  In Java, this is a "Label" object.
*/
...
/**
  A TextBox is a multi-line editable text area.
*/
...
/**
 A List provides a vertically arranged set of selectable items.
*/
...
/**
  A Listbox provides a pop-up menu displaying a set of Listbox_Items.
  Unlike a GLUI_List, only the currently selected item is shown onscreen
  unless the user is selecting a new item.
  
  You use a Listbox by creating it, then calling add_item to add various
  strings and IDs for the options.
*/
...
/**
  ...
  The live_var output of this rotation is a 4x4 rotation matrix,
  stored as 16 floats in a 
*/
