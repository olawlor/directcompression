...
#include "ogl/ppm.h" /* for screenshots */
...
void physics_display(void) 
{
	...
	if (lib.key_toggle['b']) { /* bullet time! */
		lib.dt*=0.1;
	...
	glFinish();
	float drawstart=glutGet(GLUT_ELAPSED_TIME)*0.001f;
	physics_draw_frame(lib);
	glFinish();
	float drawelapsed=glutGet(GLUT_ELAPSED_TIME)*0.001f-drawstart;
	if (drawelapsed>10.0 && 0==getenv("NO_LOCKUP_PREVENTION")) { // Machine is basically locked up--EXIT IMMEDIATELY!
		printf("physics/world.cpp: lockup prevention--exiting program.\n");
		exit(1);
	}
	
	/* Do screenshots */
	if (lib.key_typed['n'] || lib.key_toggle['m']) {
		static int save_count=0;
		int render_w=glutGet(GLUT_WINDOW_WIDTH), render_h=glutGet(GLUT_WINDOW_HEIGHT);
		char fname[100]; sprintf(fname,"image%03d.ppm",save_count++);
		oglSavePPM(0,0,render_w,render_h,fname);
	}
	
	
	/* Show performance */
	char buf[100];
	snprintf(buf,100,"%.1f ms/frame (%.1f fps)",drawelapsed*1.0e3,1.0/drawelapsed);
...
int main(int argc,char *argv[])
{
	...
	lib.screenw=1024; lib.screenh=768;
