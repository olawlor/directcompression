	...
	:s(socket), ip(ip_), port(port_), error(0)
{
	...
	url=std::string(path_ver,0,ver_start); /* extract path in between */
	...
	size_t quest=url.find_first_of('?');
	if (quest==std::string::npos) { // no CGI params
		filepath=url;
	}
	else { // parse CGI parameters
		filepath=url.substr(0,quest);
		size_t start=quest+1;
		while (start<url.size()) 
		{ // invariant: start is first new valid letter
			size_t eq=url.find_first_of('=',start);
			size_t end=url.find_first_of('&',start);
		
			std::string name=url.substr(start,eq-start);
			std::string value="";
			if (eq!=std::string::npos) { // it's got an equals sign--add it
				value=url.substr(eq+1,end-(eq+1));
			}
			//web_log<<"Found CGI parameter '"<<name<<"'='"<<value<<"'\n";
			cgi[name]=value;
		
			if (end>=url.size()) break;
			else start=end+1;
		}
	}
}
