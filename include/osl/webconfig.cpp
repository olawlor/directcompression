...
Enter new values and hit enter or Submit:<br>
	...
	/* Check parameters */
		...
		if (client.get_url()=="/" || client.get_url()=="/"+form_name) 
		{ /* initial page request */
			send_response=true; make_form=true;
		}
		
		if (client.get_url().substr(0,2+form_name.size())=="/"+form_name+"?") 
		{ /* form data coming back */
			send_response=true;
			make_form=apply_parameters(html,client.get_url().substr(2+form_name.size()));
		}
