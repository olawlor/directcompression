...
/* Client and request info access: */
	...
	/** Return the entire URL the client has requested, like "/foo/bar.cgi?baz=3"
	*/
	const std::string get_url(void) const {return url;}
	
	/** Return the filename the client has requested, like "/foo/bar.jpg"
	*/
	const std::string get_filepath(void) const {return filepath;}

	/** Look up the value of the client's HTTP header line with this keyword, or empty string if none. 
	Examples:
		std::string browser=client.get_header("User-Agent");
		std::string from=client.get_header("Referer");
	*/
	std::string get_header(const std::string &keyword) {return header[keyword];}
	
	/** Look up the value of this keyword in the client's CGI header
	*/
	std::string get_cgi(const std::string &keyword) {return cgi[keyword];}
...
private:
	...
	std::string url; /* GET ... HTTP/1.x */
	std::string filepath; /* part of URL before '?' */
	std::map<std::string,std::string> header; /**< HTTP header names and values */
	std::map<std::string,std::string> cgi; /**< CGI names and values */
	const char *error;
};
